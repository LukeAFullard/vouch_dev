...
1.  **Privacy Risk (GDPR):** The `Logger` captures all arguments by default. **Mitigation:** `TraceSession` now supports `redact_args` to mask specific argument names (e.g., `password`). However, automatic PII detection or value-based redaction is still a roadmap item.
