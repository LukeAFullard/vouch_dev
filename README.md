...
Vouch now supports auditing for major class constructors like `pd.DataFrame()` and `pd.Series()` using dynamic subclassing. This preserves `isinstance()` compatibility while ensuring audit trails are complete.
...
Note: Some internal or minor class constructors might still be unwrapped to maintain library stability.
